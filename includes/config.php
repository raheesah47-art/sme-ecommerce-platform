...
$scheme=(!empty($_SERVER['HTTPS'])&&$_SERVER['HTTPS']!=='off')?'https':'http';$host=$_SERVER['HTTP_HOST']??'localhost';$dir=str_replace('\\','/',dirname($_SERVER['SCRIPT_NAME']??''));$base=preg_replace('#/(pages|sme|admin)(/.*)?$#','',$dir);define('SITE_URL',$scheme.'://'.$host.rtrim($base,'/'));
